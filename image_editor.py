...
def changeBrightness(canvas, brightnessWindow, brightnessSlider, previousVal):
    if canvas.data.brightnessWindowClose == True:
        ...
        if canvas.data.image != None and brightnessWindow.winfo_exists():
            ...
            scale = (sliderVal - previousVal) / 200.0
...
def buttonsInit(root, canvas):
    ...
    colourPopButton = Button(toolKitFrame, text="Colour Pop", background=backgroundColour, width=buttonWidth, height=buttonHeight, command=lambda: colourPop(canvas))
    ...
    rotateButton = Button(toolKitFrame, text="Rotate", background=backgroundColour, width=buttonWidth, height=buttonHeight, command=lambda: transpose(canvas))
    rotateButton.grid(row=7, column=0)
    drawButton = Button(toolKitFrame, text="Draw", background=backgroundColour, width=buttonWidth, height=buttonHeight, command=lambda: drawOnImage(canvas))
